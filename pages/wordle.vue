<template>
  <div
    class="w-full min-h-full h-max dark flex flex-col gap-4 items-center p-8 px-4 bg-neutral-900 text-neutral-200 select-none"
  >
    ...
    <div ref="boardRef" class="bg-neutral-900 p-4 flex flex-col gap-4">
      <div
        class="text-neutral-400 flex justify-between text-sm"
        v-if="screenshotting"
      >
        <span class="">https://games.waradu.dev</span>
        <span class="text-neutral-200" v-if="state != GameState.PLAYING">
          "{{ word.toUpperCase() }}"
        </span>
      </div>
      <div class="flex flex-col gap-2 perspective-1000">
        <div v-for="row in guessCount" class="flex gap-2">
          ...
          >
            <div
              class="absolute inset-0 flex items-center justify-center text-xl lg:text-2xl border-4 border-neutral-800 bg-transparent text-neutral-200 rounded-xl backface-hidden"
              :class="`reset row-${row}`"
            >
              {{
                row - 1 == currentLine
    ...
    <div class="flex flex-col gap-1 lg:gap-2 items-center w-full max-w-[800px]">
      ...
      >
        <div
          v-for="key in row"
          :key="key"
          class="h-12 flex justify-center items-center rounded-xl transition-colors"
          :class="[
    ...
    <div
      class="flex flex-col items-center justify-center absolute inset-0 z-10 bg-neutral-900/80 backdrop-blur-sm text-md gap-4"
      v-if="modalOpen"
    >
      <div v-if="state != GameState.PLAYING">
        {{ `The answer was: ${word}` }}
      </div>
      <div class="bg-neutral-800 rounded-xl p-4 px-8 text-xl" @click="reset">
        New Game
      </div>
      <div
        class="p-2 bg-neutral-800 active:bg-neutral-700 hover:bg-neutral-700 rounded-md absolute right-2 top-2"
